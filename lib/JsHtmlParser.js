	...
	function ontoken(token, buffer) {
		if (token) {
			var preceding = token.match ? buffer.substr(0, token.match.index)
					: '';
			if (preceding) {
				appendContext(preceding);
			}
			switch (token.category) {
			case 'JsStringEscaped':
				appendContext(token.match[0]);
				break;
			...
			case 'AnchorGroupOpen':
				...
				appendContext('stream.write(html.encode(');
				break;
			...
			case 'AnchorInlineOpen':
				...
				appendContext('if(typeof ' + token.match[1]
						+ ' == "undefined") stream.write('
						+ JSON.stringify(token.match[0]) + ');\n');
				appendContext('else stream.write(html.encode(' + token.match[1]);
				break;
